...
The nozzle geometry was modelled with a conical convergent and a Rao parabolic divergent\cite{rao_article}. For the convergent a realistic cone angle of $\alpha_{con} = 30$\textdegree $\,$ was chosen from which its length can be computed.
\begin{equation}
...
The blowdown ratios $B_{pr}$ are initially chosen assuming negligible losses and according to the boundaries imposed by the combustion chamber, as stated in \autoref{subsec:input_data}. This particular element deeply influences the whole dynamics and there were no reasons to set two different $B_{pr}$. % This last value is the main contributing factor to this value, so it is set equal between the two tanks.

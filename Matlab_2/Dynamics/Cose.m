...
for i = 1 : length(OF_vet)
    for j = 1 : length(p_c_vet)
        OF = OF_vet(i);
        p_c = p_c_vet(j);
        output = cea(CEA('problem','rkt','nfz',2,'o/f',OF,'sup',eps,'case','DRY-007','p,bar',p_c,'reactants','fuel','RP-1(L)','C',1,'H',1.9423,'wt%',100,'oxid','O2(L)','O',2,'wt%',100,'output','massf','transport','trace',1e-10,'end'));
...
%% C_star costante ioukjjkiouhjbnkjiuhjnkijn
clear
close all
...
p_c_i = 50e5;       % Pa
p_c_min = 20e5;     % Pa
rho_f = 807;        % kg/m^3 https://tsapps.nist.gov/publication/get_pdf.cfm?pub_id=832303
rho_ox = 1143;      % kg/m^3 dal nist
L_star = 1.143;     % m
h = 2;              % m
d = 1;              % m
g0 = 9.81;          % m/s^2
...
T_ox_i = 90;        % K
B_f = 2.6746;       % -
B_ox = 2.682;      % - blowdown ratio del pressurizzante


% Dimensionamento a ritroso
T_i = T_i_n/lambda;
V_tot = pi*d^2*h/4;
output = cea(CEA('problem','rkt','nfz',2,'o/f',OF_i,'sup',eps,'case','DRY-007','p,bar',p_c_i/1e5,'reactants','fuel','RP-1(L)','C',1,'H',1.9423,'wt%',100,'oxid','O2(L)','O',2,'wt%',100,'output','massf','transport','trace',1e-10,'end'));
...
while valido
    ...
    output = cea(CEA('problem','rkt','nfz',2,'o/f',OF_new,'sup',eps,'case','DRY-007','p,bar',p_c_new/1e5,'reactants','fuel','RP-1(L)','C',1,'H',1.9423,'wt%',100,'oxid','O2(L)','O',2,'wt%',100,'output','massf','transport','trace',1e-10,'end'));
    c_t_new = output.froz.cf(end);
    T_c_new = output.froz.temperature(1);
    I_sp_new = output.froz.isp(end);
    gamma_new = output.froz.gamma(1);
...
T = T(~isnan(T));
I_tot = sum(T)*dt
(V_f + V_ox + V_p_f_i + V_p_ox_i)/V_tank_tot*100
cont

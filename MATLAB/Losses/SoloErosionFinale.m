...
I_sp_re_i= output.froz.isp_vac(end);
...
while true
    ...
    I_sp_re_new=output.froz.isp_vac(end);
...
title("Thrust comparioson", "Interpreter", "latex", "FontSize", fs_t)
...
xx=1:length(T_re);
figure
plot(xx, ones(length(T_re),1)*A_t_sal(1), LineWidth=lw)
grid minor
hold on
...
xlabel("time $[s]$", "Interpreter", "latex", "FontSize", fs_ax)
legend("Initail throat area", "Throat area","Interpreter", "latex", "FontSize", fs_leg)
ylim([1.03e-4 1.09e-4])
...
title("Pressure chamber comparison", "Interpreter", "latex", "FontSize", fs_t)
ylabel("$p_c\; [Pa]$", "Interpreter", "latex", "FontSize", fs_ax)
xlabel("time $[s]$", "Interpreter", "latex", "FontSize", fs_ax)
legend("Real pressure chamber", "Ideal pressure chamber","Interpreter", "latex", "FontSize", fs_leg)

...
K_f = 1 + f*L_feed_f/d_feed + A_feed/(A_inj_f_tot*C_d);
K_ox = 1 + f*L_feed_ox/d_feed + A_feed/(A_inj_ox_tot*C_d);

dp_f = 0.5*rho_f*u_feed_f^2*K_f;
dp_ox = 0.5*rho_ox*u_feed_ox^2*K_ox;

p_f_i = p_c_i + dp_f;
p_ox_i = p_c_i + dp_ox;

V_He_f_i = R*T_He_f_i/(M_m_He*p_f_i);
...
V_He_f = [V_He_f_i nan(1, length(tvet) - 1)];
...
while valido
    ...
    V_He_f_new = V_He_f(cont)  + dV_f;
    V_He_ox_new = V_He_ox(cont)  + dV_ox;
    
    p_f_new = p_f(cont)*(V_He_f(cont)/V_He_f_new)^k_He;
    p_ox_new = p_ox(cont)*(V_He_ox(cont)/V_He_ox_new)^k_He;
    
    m_f_new = sqrt(2*rho_f*A_feed^2*(p_f_new - p_c(cont))/K_f);
    m_ox_new = sqrt(2*rho_ox*A_feed^2*(p_ox_new - p_c(cont))/K_ox);
    
    V_p = V_p + dV_f + dV_ox;
    if V_p + V_He_ox_i + V_He_f_i > V_tank_tot
        valido = 0;
        disp("Termine per volume occupato massimo raggiunto")
        continue
    end
    ...
    OF_new = m_ox_new/m_f_new;
    output = cea(CEA('problem','rkt','nfz',2,'o/f',OF_new,'sup',eps,'case','Cazzo','p,bar',p_c(cont)/1e5,'reactants','fuel','RP-1(L)','C',1,'H',1.9423,'wt%',100,'oxid','O2(L)','O',2,'wt%',100,'output','massf','transport','trace',1e-10,'end'));
    T_c_new = output.froz.temperature(1);
    gamma_new = output.froz.gamma(1);
    p_c_new = (T_c(cont)*p_c(cont)^((1 - gamma_new)/gamma_new)/T_c_new)^(gamma_new/(1 - gamma_new));
    Rapp_T = (T_c(cont)/T_c_new)^(gamma_new/(1 - gamma_new))
